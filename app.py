“”“MediSmart AI — Backend Flask API”””
...
params = []
if specialite:
    query += ' AND specialite = ?'; params.append(specialite)
if pays:
    query += ' AND pays = ?'; params.append(pays)
if gravite:
    query += ' AND gravite = ?'; params.append(gravite)
if search:
...
# Générer ordonnance automatique
...
‘Neurologie’: [‘Gabapentine 300mg — 1cp x3/jour’, ‘Vitamine B12 1000µg — 1cp/jour’],
‘Pneumologie’: [‘Salbutamol spray — 2 bouff. si besoin’, ‘Béclométasone 200µg — 2 bouff. x2/jour’],
‘Pédiatrie’: [‘Paracétamol 250mg — 1 supp. x3/jour’, ‘Vitamine D 800UI — 1cp/jour’],
‘Oncologie’: [‘Selon protocole oncologique spécifique’, ‘Antiémétiques selon tolérance’],
‘default’: [‘Paracétamol 1g — 1cp x3/jour si douleur’, ‘Repos et hydratation suffisante’]
}
return meds.get(specialite, meds[‘default’])

def generate_ai_analysis(data):
nom = data.get(‘nom’, ‘Patient’)
sp = data.get(‘specialite’, ‘Médecine générale’)
gravite = data.get(‘gravite’, ‘modérée’)
symptomes = (data.get(‘symptomes’) or ‘’)[:100]
temp = data.get(‘temperature’)
fc = data.get(‘fc’)
alerts = []
if temp and float(temp) > 38: alerts.append(‘⚠️ Fièvre détectée — bilan infectieux conseillé.’)
if fc and int(fc) > 100: alerts.append(‘⚠️ Tachycardie — surveillance cardiaque recommandée.’)
if fc and int(fc) < 50: alerts.append(‘⚠️ Bradycardie — consultation urgente.’)
alert_text = ’ ’.join(alerts)
return f”Analyse IA — Patient {nom}, {sp}. Gravité {gravite}. Symptômes : {symptomes}. {alert_text} Protocole thérapeutique recommandé avec réévaluation dans 4 semaines.”
